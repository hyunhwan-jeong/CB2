...
#' 
#' @param lib_path The path of annotation file.
#'
#' @param design  A data.frame contains study design. It has to contain `fastq_path` and `sample_name`.
#'
#' @return It will return a list, and the list contains two elements. 
#'   The first elements (`count`) is a data.frame contains the result of the quantification for each sample. 
...
run_sgrna_quant <- function(lib_path, design) {
    # `design` has to be data.frame `design` has to have four columns: sample_name, group, fastq_path
    if(!all(c("sample_name", "fastq_path") %in% colnames(design))) {
        stop("The design data.frame should have `sample_name` and `fastq_path` columns.")    
    }
    
    if(!file.exists(lib_path)) {
        stop("the library annotation file (FASTA) is not existed.")
    }
    if(!all(file.exists(design$fastq_path))) {
        stop("the some of sample FASTQ files are not existed.")
    }
...
#' @param sgcount A data.frame contains 
#'
#' @param design A data.frame contains study design. It has to contain `group`
#' @param group_a The first group to be in the test.
#' @param group_b The second group to be in the test.
#'
#' @importFrom magrittr %>%
#' @importFrom tibble tibble
#' @importFrom dplyr arrange
#' @return a data.frame contains the sgRNA-level test result, and the data.frame contains these columns: 
#' \itemize{
...
run_estimation <- function(sgcount, design, group_a, group_b) {
    ...
    if(!all(c("sample_name", "group") %in% colnames(design))) {
        stop("The design data.frame should have `sample_name` and `group` columns.")    
    }
    ...
    if(!all(group_a %in% design$group)) {
        stop("group_a must be one of groups in the design data.frame.")
    }
    if(!all(group_b %in% design$group)) {
        stop("group_b must be one of groups in the design data.frame.")
    }
...
#' 
#' @param sgrna_stat a data.frame generated by `run_estimation`
#'
#' @importFrom magrittr %>%
#' @importFrom tibble tibble
#' @importFrom dplyr group_by
#' 
#' @return a data.frame contains the gene-level test result, and the data.frame contains these columns: 
#' \itemize{
...
measure_gene_stats <- function(sgrna_stat) {
    if(!all(c("gene", "cpm_a", "cpm_b", "logFC", "p_ts", "p_pa", "p_pb") %in% colnames(sgrna_stat))) {
        stop("It seems that the `sgrna_stat` does not contain any result of statistical test.")
    }
    sgrna_stat %>% dplyr::group_by(gene) %>%
        dplyr::summarise(
            n_sgrna = n(),
            cpm_a = mean(cpm_a),
            cpm_b = mean(cpm_b),
            logFC = mean(logFC),
            p_ts = ifelse(n() >
                              1, metap::sumlog(p_ts)$p, sum(p_ts)),
